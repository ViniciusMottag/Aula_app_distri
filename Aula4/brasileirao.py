...
def nome_campeao(dados):
    id_time_campeao = id_campeao(dados)
    return nome_do_time(dados,id_time_campeao)
...
def qtos_libertadores(dados):
    ...
    lista_split=faixa.split('-')
    #return int(lista_split[1])
...
def ids_dos_melhor_classificados(dados,numero_de_times):
    ...
    while contador < numero_de_times:
        resposta.append(classificacao[contador])
        contador = contador + 1
    return resposta
...
def nomes_classificados_libertadores(dados):
    nome_classifica=classificados_libertadores(dados)
    id_nome=[]
    for valor in nome_classifica:
        id_nome.append(nome_do_time(dados,valor))
    return id_nome
...
def nomes_dos_times_de_um_jogo(dados,id_jogo):
    nomes_times_jogo=(ids_dos_times_de_um_jogo(dados,id_jogo))
    nomes_times=[]
    for valor in nomes_times:
        nomes_times.append(nome_do_times(dados,valor))
    return nomes_times
...
class TestClientes(unittest.TestCase):
    ...
    def test_003_qtos_libertadore0s(self):
        dados = pega_dados()
